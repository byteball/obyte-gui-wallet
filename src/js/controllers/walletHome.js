...
angular
	...
	.controller("walletHomeController", function (
		$scope,
		$rootScope,
		$timeout,
		$filter,
		...
		this.testnetName =
			constants.alt === "2" ? "[NEW TESTNET]" : "[TESTNET]";
		...
		var disablePaymentRequestListener = $rootScope.$on(
			"paymentRequest",
			function (
				event,
				address,
		...
		var disableDataPromptListener = $rootScope.$on("dataPrompt", function (
			event,
			dataPrompt
		) {
		...
		var disablePaymentUriListener = $rootScope.$on("paymentUri", function (
			event,
			uri
		) {
		...
		var disableAddrListener = $rootScope.$on(
			"Local/NeedNewAddress",
			function () {
				self.setAddress(true);
			}
		);

		var disableFocusListener = $rootScope.$on(
			"Local/NewFocusedWallet",
			function () {
				self.addr = {};
				self.resetForm();
				self.setAddress();
		...
		var disableTabListener = $rootScope.$on("Local/TabChanged", function (
			e,
			tab
		) {
		...
		var disableOngoingProcessListener = $rootScope.$on(
			"Addon/OngoingProcess",
			function (e, name) {
		...
		$scope.openDestinationAddressModal = function (wallets, address) {
			...
			var ModalInstanceCtrl = function ($scope, $modalInstance) {
				...
				$scope.toggleSelectAddressbook = function (addr) {
					$scope.selectedAddressbook[addr] = $scope
						.selectedAddressbook[addr]
						? false
						: true;
				};
				...
				$scope.selectWallet = function (walletId, walletName) {
					...
					addressService.getAddress(
						walletId,
						false,
						function onGotAddress(err, addr) {
							$scope.gettingAddress = false;

							if (err) {
								self.error = err;
								breadcrumbs.add(
									"openDestinationAddressModal getAddress err: " +
										err
								);
								$modalInstance.dismiss("cancel");
								return;
			...
			modalInstance.result.finally(function () {
				...
				var m = angular.element(
					document.getElementsByClassName("reveal-modal")
				);
		...
		$scope.openSharedAddressDefinitionModal = function (address) {
			...
			var ModalInstanceCtrl = function ($scope, $modalInstance) {
				...
				$scope.shared_address_cosigners =
					indexScope.shared_address_cosigners;

				var walletGeneral = require("ocore/wallet_general.js");
				var walletDefinedByAddresses = require("ocore/wallet_defined_by_addresses.js");
				walletGeneral.readMyAddresses(function (arrMyAddresses) {
					walletDefinedByAddresses.readSharedAddressDefinition(
						address,
						function (arrDefinition, creation_ts) {
							walletDefinedByAddresses.readSharedAddressPeers(
								address,
								function (assocPeerNamesByAddress) {
									$scope.humanReadableDefinition = correspondentListService.getHumanReadableDefinition(
										arrDefinition,
										arrMyAddresses,
										[],
				...
				$scope.sendPayment = function (address, amount, asset) {
					if (
						...
						return console.log(
							"i do not own anything of asset " + asset
						);
			...
			var disableCloseModal = $rootScope.$on("closeModal", function () {
				breadcrumbs.add(
					"openSharedAddressDefinitionModal on closeModal"
				);
				modalInstance.dismiss("cancel");
			});
		...
		this.shareAddress = function (addr) {
			if (isCordova) {
				...
				window.plugins.socialsharing.shareWithOptions(
					{
						message:
							"My Obyte address " + self.protocol + ":" + addr,
						subject:
							"My Obyte address" /*, url: self.protocol +  ':' + addr*/,
					},
					function () {},
					function () {}
				);
		...
		this.openCustomizedAmountModal = function (addr) {
			...
			var ModalInstanceCtrl = function ($scope, $modalInstance) {
				...
				$scope.buttonLabel = gettextCatalog.getString(
					"Generate QR Code"
				);
				...
				$scope.submitForm = function (form) {
					if ($scope.index.arrBalances.length === 0)
						return console.log(
							"openCustomizedAmountModal: no balances yet"
						);
					var amount = form.amount.$modelValue;
					var assetInfo =
						$scope.index.arrBalances[$scope.index.assetIndex];
					var asset = assetInfo.asset;
					if (!asset) throw Error("no asset");
					var amountInSmallestUnits = profileService.getAmountInSmallestUnits(
						amount,
						asset
					);
					$timeout(function () {
		...
		function claimTextCoin(mnemonic, addr) {
			...
			wallet.receiveTextCoin(mnemonic, addr, function (err, unit, asset) {
				$timeout(function () {
					...
					if (asset) {
						var disableBalanceListener = $rootScope.$on(
							"Local/BalanceUpdated",
							function (assocBalances) {
								var assetIndex = lodash.findIndex(
									indexScope.arrBalances,
									{
										asset: asset,
									}
								);
		...
		var disableClaimTextcoinListener = $rootScope.$on(
			"claimTextcoin",
			function (event, mnemonic) {
				breadcrumbs.add(
					"received claimTextcoin event with mnemonic: " +
						mnemonic.substr(0, 10) +
						"..."
				);
				var addr =
					self.addr[
						profileService.focusedClient.credentials.walletId
					];
				if (addr) {
					claimTextCoin(mnemonic, addr);
				} else {
		...
		this.setSendPaymentFormInputs = function () {
			 ...
			 *
			 */
			Object.defineProperty($scope, "_amount", {
				get: function () {
			...
			Object.defineProperty($scope, "_address", {
				get: function () {
			...
			var prefix =
				fc.credentials.m > 1
					? gettextCatalog.getString(
							"Could not create payment proposal"
					  )
		...
		function getShareMessage(amount, mnemonic, asset) {
				...
				if (assetInfo) {
					...
					if (network.exchangeRates[pair]) {
						usd_amount_str =
							...
							(
								(amount /
									Math.pow(10, assetInfo.decimals || 0)) *
								network.exchangeRates[pair]
							).toLocaleString([], { maximumFractionDigits: 2 }) +
							" USD)";
					}
			...
			return {
				message:
					...
					usd_amount_str +
					(is_private
						? ".  If you don't have a Obyte wallet yet, install it from https://obyte.org."
						: ": https://obyte.org/#textcoin?" + mnemonic),
				subject: "Obyte user beamed you money",
			};
		}

		this.openShareTextcoinModal = function (
			addr,
			mnemonic,
			amount,
			asset,
			...
			if (!addr && isCordova) {
				...
				window.plugins.socialsharing.shareWithOptions(
					lodash.assign(getShareMessage(amount, mnemonic, asset), {
			...
			var ModalInstanceCtrl = function ($scope, $modalInstance) {
				...
				$scope.shareToEmail = function () {
					window.plugins.socialsharing.shareViaEmail(text, subject, [
						addr,
					]);
					$modalInstance.close();
				};
		...
		this.validateAADefinition = function () {
			...
			lodash.debounce(function () {
				parse_ojson.parse(self.definition, function (
					err,
					arrDefinition
				) {
					if (err) {
					...
					aa_validation.validateAADefinition(arrDefinition, function (
						err
					) {
		...
		this.validateTextLength = function () {
			var form = $scope.sendDataForm;
			form.content.$setValidity(
				"validLength",
				!(self.content && self.content.length > 140)
			);
		};
		...
		this.onAddressChanged = function () {
			...
			var accountValidationResult = aliasValidationService.validate(
				address
			);
			if (!accountValidationResult.isValid) return;
			...
			if (
				bb_address === "unknown" ||
				bb_address === "none" ||
				!ValidationUtils.isValidAddress(bb_address)
			)
				return;
		...
		function readAADefinitionsWithBaseDefinitions(address, handleResult) {
			...
			aa_addresses.readAADefinitions([address], function (rows) {
				if (rows.length === 0) return handleResult([]);
				if (!rows[0].base_aa)
					// regular AA
					return handleResult(rows);
				// else, parameterized AA
				aa_addresses.readAADefinitions([rows[0].base_aa], function (
					base_rows
				) {
		...
		function updateAADocs() {
			...
			var arrBaseDefinition = JSON.parse(
				row.base_definition || row.definition
			);
			...
			require("ocore/uri.js").fetchUrl(doc_url, function (err, response) {
				if (err) return console.log("fetching doc_url failed: " + err);
				try {
					...
					if (self.aa_description.length > 200)
						self.aa_truncated_description =
							self.aa_description.substr(0, 180) + "...";
				} catch (e) {
					return console.log(
						"failed to parse doc_url response: " +
							e +
							", the response was: " +
							response
					);
		...
		this.showDataFieldSuggestions = function (
			currentElem,
			index,
			arrayOfElements
		) {
		...
		this.onMultiAddressesChanged = function () {
			...
			if (errors.validAddresses)
				// not valid yet
				return;
			var arrAddresses = getOutputsForMultiSend().map(function (output) {
		...
		function addBounceFees(asset, amount, address) {
			if (!self.added_bounce_fees) self.added_bounce_fees = [];
			if (
				self.added_bounce_fees.filter(function (feeInfo) {
					return feeInfo.asset === asset;
				}).length === 0
			) {
				var feeInfo = {
					asset: asset,
					address: address,
					amount: amount,
					display_amount: profileService.formatAmountWithUnit(
						amount,
						asset,
						{ dontRound: true }
					),
		...
		function dryRunPrimaryAATrigger(
			trigger,
			aa_address,
			arrDefinition,
			handleResponses
			...
			if (!conf.bLight) {
				...
				return aa_composer.dryRunPrimaryAATrigger(
					trigger,
					aa_address,
					arrDefinition,
					function (arrResponses) {
						if (constants.COUNT_WITNESSES === 1) {
							// the temp unit might rebuild the MC
			...
			network.requestFromLightVendor(
				"light/dry_run_aa",
				{ trigger: trigger, address: aa_address },
				function (ws, request, response) {
					if (response.error) return handleResponses(response.error);
		...
		function updateAAResults() {
			...
			var strDefinition =
				self.aa_destinations[0].base_definition ||
				self.aa_destinations[0].definition;
			var data_fields_to_input = [
				...new Set(strDefinition.match(target_to_find)),
			];
			if (data_fields_to_input.length) {
				...
				self.aa_data_fields_defined = lodash.difference(
					threeEntriesArray,
					moreEntriesArray
				); // filter 3 entry words with filter words;
			}

			var row = self.aa_destinations[0];
			var aa_address = row.address;
			var arrDefinition = JSON.parse(row.definition);
			var arrBaseDefinition = row.base_definition
				? JSON.parse(row.base_definition)
				: arrDefinition;
			var bounce_fees = arrBaseDefinition[1].bounce_fees || {
				base: constants.MIN_BYTES_BOUNCE_FEE,
			};
			if (!bounce_fees.base) {
			...
			var address =
				indexScope.shared_address ||
				self.addr[profileService.focusedClient.credentials.walletId];
			if (self.from_address && !indexScope.shared_address)
				address = self.from_address;
			if (!address) throw Error("no address");
			var trigger = { outputs: {}, address: address };
			if ($scope.home.feedvaluespairs.length > 0) trigger.data = {};
			...
			if (amount < (bounce_fees[asset] || 0)) {
				if (amount === 0) {
					...
					self.custom_amount_error =
						gettext("Minimum amount") +
						" " +
						profileService.getAmountInDisplayUnits(
							bounce_fees[asset],
							asset
				...
				self.custom_amount_error = null;
				if (amount > 0) form.amount.$setValidity("aaBounceFees", true);
			}
			if (!amount) return console.log("no amount yet");
			...
			dryRunPrimaryAATrigger(
				...
				function (err, arrResponses) {
					...
					var arrSecondaryAAAdresses = arrResponses.map(function (
						objResponse
					) {
						return objResponse.aa_address;
					});
					arrResponses.forEach(function (objResponse) {
						if (objResponse.bounced)
							results.push(
								gettext("Bounce the request") +
									(objResponse.response.error
										? ": " + objResponse.response.error
										: "")
							);
						if (
							objResponse.updatedStateVars &&
							objResponse.updatedStateVars[aa_address]
						) {
							for (var variable in objResponse.updatedStateVars[
								aa_address
							]) {
						...
						if (objResponse.response.responseVars) {
							for (var variable in objResponse.response
								.responseVars)
								responseVars.push({
									variable: variable,
									value:
										objResponse.response.responseVars[
											variable
										],
								});
						}
						if (!objResponse.objResponseUnit) return;
						objResponse.objResponseUnit.messages.forEach(function (
							message
						) {
							if (message.app === "payment") {
								message.payload.outputs.forEach(function (
									output
								) {
									if (
										output.address ===
										objResponse.aa_address
									)
										// change
										return;
									if (
										arrSecondaryAAAdresses.indexOf(
											output.address
										) >= 0
									)
										// to another AA
									...
									var display_address =
										output.address ===
										objResponse.trigger_address
											? gettext("you")
											: output.address;
									var display_amount = profileService.formatAmountWithUnit(
										output.amount,
										message.payload.asset || "base",
										{ dontRound: true }
									);
									results.push(
										gettext("Send") +
								...
								results.push(
									gettext("Define a new asset with cap ") +
										(message.payload.cap ||
											gettext("unlimited"))
		...
		function getOutputsForMultiSend() {
			...
			var assetInfo = $scope.index.arrBalances[$scope.index.assetIndex];
			var asset = assetInfo.asset;
			var addressesValue = form.addresses.$modelValue
				? form.addresses.$modelValue
				: "";
		...
		this.submitPayment = function () {
			...
			if (
				$scope.mtab == 2 &&
				!isMultipleSend &&
				!form.address.$modelValue
			) {
				// clicked 'share via message' button
			...
			if (
				assetInfo.is_private &&
				self.aa_destinations &&
				self.aa_destinations.length > 0
			)
				return self.setSendError(
					gettext(
						"It is not allowed to send private assets to autonomous agents"
					)
				);
			if (
				self.binding &&
				...
				return self.setSendError(
					gettext(
						"It is not allowed to bind conditions to payments to autonomous agents"
					)
				);

			if (
				conf.bLight &&
				indexScope.copayers.length > 1 &&
				indexScope.onGoingProcess["Syncing"]
			)
				//wait for sync before sending
				return self.setSendError(
					gettext("wait for sync to complete before sending payments")
				);

			if (isMultipleSend) {
				if (assetInfo.is_private)
					return self.setSendError(
						"private assets can not be sent to multiple addresses"
					);
				var outputs = getOutputsForMultiSend();
				var current_payment_key = form.addresses.$modelValue.replace(
					/[^a-zA-Z0-9]/g,
					""
				);
			} else {
				var address = form.address.$modelValue;
				var recipient_device_address =
					assocDeviceAddressesByPaymentAddress[address];
				var amount = form.amount.$modelValue;
				...
				if (isTextcoin)
					address =
						"textcoin:" +
						(address ? address : Date.now() + "-" + amount);
				if (asset === "base") amount *= unitValue;
				else if (asset === constants.BLACKBYTES_ASSET)
					amount *= bbUnitValue;
				else if (assetInfo.decimals)
					amount *= Math.pow(10, assetInfo.decimals);
			...
			if (current_payment_key === self.current_payment_key)
				return $rootScope.$emit(
					"Local/ShowErrorAlert",
					"This payment is already under way"
				);
			self.current_payment_key = current_payment_key;

			indexScope.setOngoingProcess(gettext("sending"), true);
			$timeout(function () {
				if (!isMultipleSend && accountValidationResult.isValid) {
					// try to replace validation result with attested BB address
					var attestorKey = accountValidationResult.attestorKey;
					var account = accountValidationResult.account;
					var bb_address = aliasValidationService.getBbAddress(
						attestorKey,
					...
					if (!isEmail) {
						if (bb_address === "unknown" || bb_address === "none") {
							...
							throw Error(
								"unrecognized bb_address: " + bb_address
							);
						}
					} else {
						if (bb_address === "unknown") {
				...
				profileService.requestTouchid(function (err) {
					...
					if (self.binding) {
						if (isTextcoin) {
							...
							return self.setSendError(
								"you can send bound payments to Obyte adresses only"
							);
						}
						...
						useOrIssueNextAddress(
							...
							function (addressInfo) {
								...
								if (self.binding.type === "reverse_payment") {
									...
									if (
										self.binding.oracle_address ===
										configService.TIMESTAMPER_ADDRESS
									)
										self.binding.feed_value = parseInt(
											self.binding.feed_value
										);
									var arrExplicitEventCondition = [
										"in data feed",
										[
											[self.binding.oracle_address],
											self.binding.feed_name,
									...
									if (self.binding.feed_type === "explicit")
										...
										self.binding.feed_type === "merkle"
									)
										arrEventCondition = arrMerkleEventCondition;
									else if (
										self.binding.feed_type === "either"
									)
										arrEventCondition = [
											"or",
											[
												arrMerkleEventCondition,
												arrExplicitEventCondition,
											],
										];
									else
										throw Error(
											"unknown feed type: " +
												self.binding.feed_type
										);
									var arrDefinition = [
										"or",
										[
					...
					function composeAndSend(to_address) {
						...
						if (fc.credentials.m < fc.credentials.n)
							$scope.index.copayers.forEach(function (copayer) {
								if (copayer.me || copayer.signs)
									arrSigningDeviceAddresses.push(
										copayer.device_address
									);
							});
						else if (indexScope.shared_address)
							arrSigningDeviceAddresses = indexScope.copayers.map(
								function (copayer) {
						...
						var opts = {
							...
							spend_unconfirmed: configWallet.spendUnconfirmed
								? "all"
								: "own",
						...
						if (!isMultipleSend) {
							...
							if (asset !== "base") opts.asset_outputs = outputs;
							else opts.base_outputs = outputs;
						}
						// add bounce fees if necessary
						if (
							self.added_bounce_fees &&
							self.added_bounce_fees.length > 0
						) {
							var other_asset_added_bounce_fees = self.added_bounce_fees.filter(
								function (feeInfo) {
									return (
										feeInfo.asset !== "base" &&
										feeInfo.asset !== asset
							...
							if (other_asset_added_bounce_fees.length > 0) {
								...
								return self.setSendError(
									"cannot add bounce fees in asset " +
										other_asset_added_bounce_fees[0].asset
								);
							}
							if (asset !== "base") {
								// add base_outputs to pay for bounce fees
								if (!isMultipleSend) {
									opts.asset_outputs = [
								...
								opts.base_outputs = self.added_bounce_fees
									.filter(function (feeInfo) {
										return feeInfo.asset === "base";
									})
									.map(function (feeInfo) {
										return {
						...
						var filePath;
						if (assetInfo.is_private) {
							opts.getPrivateAssetPayloadSavePath = function (
								cb
							) {
								self.getPrivatePayloadSavePath(function (
									fullPath,
									cordovaPathObj
								) {
									filePath = fullPath
										...
										? cordovaPathObj.root +
										  cordovaPathObj.path +
										  "/" +
										  cordovaPathObj.fileName
										: null;
									cb(fullPath, cordovaPathObj);
								});
							};
						}
						if (objDataMessage) opts.messages = [objDataMessage];
						fc.sendMultiPayment(opts, function (
							err,
							unit,
							mnemonics
						) {
							...
							if (err) {
								if (typeof err === "object") {
									...
									err =
										"This is a private asset, please send it only by clicking links from chat";
								else if (err.match(/no funded/)) {
									if (self.from_address) {
										$rootScope.$emit(
											"Local/ShowErrorAlert",
											"Not enough spendable funds on address " +
												self.from_address +
												", you may want to refill its balance"
										);
										self.resetForm(); // to enable to fill the form anew to refill this address
									} else
										err =
											"Not enough spendable funds, make sure all your funds are confirmed";
								} else if (
									err.match(
							...
							if (original_address) {
								var db = require("ocore/db.js");
								db.query(
									"INSERT INTO original_addresses (unit, address, original_address) VALUES(?,?,?)",
									[unit, to_address, original_address]
								);
							}
							if (recipient_device_address) {
								// show payment in chat window
								eventBus.emit(
									"sent_payment",
									recipient_device_address,
									amount || "all",
								...
								if (binding && binding.reverseAmount) {
									// create a request for reverse payment
									if (!my_address)
									...
									var paymentRequestCode =
										...
										"&asset=" +
										encodeURIComponent(
											binding.reverseAsset
										);
									var paymentRequestText =
										"[reverse payment](" +
										paymentRequestCode +
										")";
									...
									var body = correspondentListService.formatOutgoingMessage(
										paymentRequestText
									);
									correspondentListService.addMessageEvent(
										false,
										recipient_device_address,
										body
									);
									device.readCorrespondent(
										...
										function (correspondent) {
											if (
												correspondent.my_record_pref &&
												correspondent.peer_record_pref
											)
												chatStorage.store(
								...
								self.openShareTextcoinModal(
									isEmail
										? address.slice("textcoin:".length)
										: null,
									mnemonic,
									amount,
									asset,
							...
							else $rootScope.$emit("Local/SetTab", "history");
						});
					}

					function useOrIssueNextAddress(
						wallet,
						is_change,
						handleAddress
					) {
		...
		this.switchForms = function () {
			...
			if (
				this.send_multiple &&
				$scope.index.arrBalances[$scope.index.assetIndex] &&
				$scope.index.arrBalances[$scope.index.assetIndex].is_private
			)
			...
			if ($scope.assetIndexSelectorValue < 0) {
				...
				if (
					!this.feedvaluespairs ||
					(this.feedvaluespairs.length > 0 &&
						(!this.feedvaluespairs[0].name ||
							!this.feedvaluespairs[0].value))
				)
					this.feedvaluespairs = [];
			}
			$scope.mtab =
		...
		this.submitData = function () {
			...
			profileService.requestTouchid(function (err) {
				...
				if (app == "definition") {
					parse_ojson.parse($scope.home.definition, function (
						err,
						arrDefinition
					) {
						if (err) {
				...
				function sendData(value) {
					...
					fc.sendMultiPayment(
						...
						function (err, unit) {
							...
							breadcrumbs.add(
								"done submitting data into feeds " +
		...
		this.canSendExternalPayment = function () {
			if (
				$scope.index.arrBalances.length === 0 ||
				$scope.index.assetIndex < 0
			)
				// no balances yet, assume can send
				return true;
			if (!$scope.index.arrBalances[$scope.index.assetIndex])
				// no balances yet, assume can send
				return true;
			...
			var ModalInstanceCtrl = function ($scope, $modalInstance) {
				...
				$scope.arrPublicAssetInfos = indexScope.arrBalances
					...
					.map(function (b) {
						...
						if (b.asset === "base")
							...
							info.displayName =
								profileService.assetMetadata[b.asset].name;
						else info.displayName = "of " + b.asset.substr(0, 4);
						return info;
					});
				$scope.binding = {
					// defaults
				...
				if (self.binding) {
					...
					if (self.binding.type === "reverse_payment") {
						...
						$scope.binding.reverseAmount = profileService.getAmountInDisplayUnits(
							self.binding.reverseAmount,
							self.binding.reverseAsset
						);
					} else {
						$scope.binding.oracle_address =
							self.binding.oracle_address;
				...
				$scope.bind = function () {
					...
					if (binding.type === "reverse_payment") {
						...
						binding.reverseAmount = profileService.getAmountInSmallestUnits(
							$scope.binding.reverseAmount,
				...
				function () {
						...
						try {
							...
							if (paymentData) {
								...
								for (var key in paymentData) {
									$scope.home.feedvaluespairs.push({
										name: key,
							...
							$rootScope.$emit(
								"Local/ShowErrorAlert",
								"invalid data " + e.toString()
							);
							return self.resetForm();
						}
					}

					if (from_address) this.from_address = from_address;

					if (asset) {
						if ($scope.index.arrBalances.length === 0) {
							// wait till balances are set
						...
						if (assetIndex < 0) {
							$rootScope.$emit(
								"Local/ShowErrorAlert",
								"failed to find asset index of asset " + asset
							);
							return self.resetForm();
						}
					...
					if (to) {
						...
						if (recipient_device_address)
							...
							assocDeviceAddressesByPaymentAddress[
								to
							] = recipient_device_address;
						if ($scope.assetIndexSelectorValue < 0 && !asset)
							// a data form was selected
					...
					addressService.getAddress(
						profileService.focusedClient.credentials.walletId,
						false,
						function (err, my_address) {
							if (single_address && single_address !== "0") {
								var displayed_single_address = from_address
									? " " + from_address
									: "";
								var fc = profileService.focusedClient;
								if (
									...
									$rootScope.$emit(
										"Local/ShowErrorAlert",
										gettext(
											"This payment must be paid only from single-address wallet"
										) +
											displayed_single_address +
											"."
									);
									return self.resetForm();
								}
				...
				function () {
					switch (app) {
						...
						case "vote":
							$rootScope.$emit(
								"Local/ShowErrorAlert",
								"voting not yet supported via uri"
							);
							return self.resetForm();
						case "definition":
							$scope.assetIndexSelectorValue = -6;
					...
					for (var key in dataPrompt) {
						...
						$scope.home.feedvaluespairs.push(
							app === "poll"
								...
								: { name: key, value: value, readonly: true }
						);
					}
					this.switchForms();
					//	$timeout(function () {
		...
		this.resetForm = function (bKeepData) {
			...
			this.bSendAll = false;
			if (!bKeepData) this.feedvaluespairs = [];
			resetAAFields();
		...
		this.setSendAll = function () {
			...
			if (indexScope.arrBalances.length === 0) return;
		...
		this.onAddressChange = function (value) {
			...
			if (
				value.indexOf(self.protocol + ":") === 0 ||
				value.indexOf(self.bb_protocol + ":") === 0
			)
		...
		this.getPrivatePayloadSavePath = function (cb) {
			var fileName =
				"ObytePayment-" +
				$filter("date")(Date.now(), "yyyy-MM-dd-HH-mm-ss") +
				"." +
				configService.privateTextcoinExt;
			if (!isCordova) {
		...
		this.openInExplorer = function (unit) {
			...
			else if (isCordova) cordova.InAppBrowser.open(url, "_system");
		};

		this.sendAttachedFile = function ($ev) {
			home.attachedFile = $ev.target.files[0];
			if (!home.attachedFile) return;
			console.log("attached file => ", home.attachedFile);
		...
		this.openTxModal = function (btx) {
			...
			var ModalInstanceCtrl = function ($scope, $modalInstance) {
				$scope.btx = btx;
				$scope.btx.response =
					typeof btx.response === "string"
						? JSON.parse(btx.response)
						: btx.response;
				...
				$scope.isPrivate =
					indexScope.arrBalances[assetIndex].is_private;
				$scope.Math = window.Math;
				$scope.assetDecimals =
					indexScope.arrBalances[assetIndex].decimals;
				...
				$scope.shareAgain = function () {
					if ($scope.isPrivate) {
						...
						var wallet = require("ocore/wallet.js");
						indivisible_asset.restorePrivateChains(
							btx.asset,
							btx.unit,
							btx.addressTo,
							function (arrRecipientChains, arrCosignerChains) {
								self.getPrivatePayloadSavePath(function (
									...
									if (!fullPath && !cordovaPathObj) return;
									var filePath = fullPath
										...
										: cordovaPathObj.root +
										  ...
										  "/" +
										  cordovaPathObj.fileName;
									wallet.storePrivateAssetPayload(
										fullPath,
										cordovaPathObj,
										btx.mnemonic,
										arrRecipientChains,
				...
				$scope.eraseTextcoin = function () {
					(function () {
						...
						var ModalInstanceCtrl = function (
							...
							$scope.title = gettextCatalog.getString(
								"Deleting the textcoin will remove the ability to claim it back or resend"
							);
							$scope.cancel_button_class = "light-gray outline";
							$scope.loading = false;
							$scope.confirm_label = gettextCatalog.getString(
								"Confirm"
							);
						...
						modalInstance.result.finally(function () {
							...
							m.addClass(
								animationService.modalAnimated.slideOutDown
							);
						});
					})();
				};
					...
					indivisible_asset.restorePrivateChains(
						...
						function (arrRecipientChains, arrCosignerChains) {
							if (indexScope.shared_address) {
								...
								wallet_defined_by_keys.forwardPrivateChainsToOtherMembersOfWallets(
									arrCosignerChains,
									[fc.credentials.walletId],
									null,
									success
		...
		this.showCorrespondentListToReSendPrivPayloads = function (btx) {
			...
			var ModalInstanceCtrl = function (
				...
				$scope.sendPrivatePayments = function (correspondent) {
					...
					indivisible_asset.restorePrivateChains(
						...
						btx.addressTo,
						function (arrRecipientChains, arrCosignerChains) {
							wallet_general.sendPrivatePayments(
								correspondent.device_address,
								arrRecipientChains,
								true,
								null,
			...
			var modalInstance = $modal.open({
				templateUrl:
					"views/modals/correspondentListToReSendPrivPayloads.html",
				windowClass: animationService.modalAnimated.slideRight,
				controller: ModalInstanceCtrl,
			});
		...
		this.getDollarValue = function (amount, balanceObject) {
			function getResult(exchangePair) {
				if (home.bSendAll && home.from_address) return; // TEMP fix: don't show value if from specific address and "All minus fee" shown
				var result = 0;
				if (
					exchangePair === "GBYTE_USD" ||
					exchangePair === "GBB_USD"
				) {
					var amountInSmallestUnits = profileService.getAmountInSmallestUnits(
						amount,
						balanceObject.asset
					...
					result =
						(amountInSmallestUnits / 1e9) *
						home.exchangeRates[exchangePair];
					if (home.bSendAll) {
		...
		this.calculateAmount = function (amount, asset) {
			var assetInfo = indexScope.arrBalances[indexScope.assetIndex];
			if (asset === "base") return "" + amount / self.unitValue;
			else if (asset === constants.BLACKBYTES_ASSET)
		...
		this.resend = function (btx) {
			...
			if (form.amount) {
				form.amount.$setViewValue(
					"" + this.calculateAmount(btx.amount, btx.asset)
				);
				form.amount.$render();
			}
			if (form.address) {
			...
			storage.readJoint(
				db,
				btx.unit,
				{ ifFound: ifFound, ifNotFound: ifNotFound },
				false
			);
		};
