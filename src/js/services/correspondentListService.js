...
angular.module('copayApp.services').factory('correspondentListService', function($state, $rootScope, $sce, $compile, configService, storageService, profileService, go, lodash, $stickyState, $deepStateRedirect, $timeout, gettext, isCordova, pushNotificationsService) {
	...
	var payment_request_regexp = /\[.*?\]\(((?:byteball|obyte|byteball-tn|obyte-tn):([0-9A-Z]{32})(?:\?([\w=&;+%]+))?)\)/g;
	var pairing_regexp = /\[.*?\]\(((?:byteball|obyte|byteball-tn|obyte-tn):([\w\/+]{44})@([\w.:\/-]+)#(.+))\)/g;
	var textcoin_regexp = /\[.*?\]\(((?:byteball|obyte|byteball-tn|obyte-tn):textcoin\?(.+))\)/g;
	var data_regexp = /\[.*?\]\(((?:byteball|obyte|byteball-tn|obyte-tn):data\?(.+))\)/g;
